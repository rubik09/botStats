...
export class StatsRepository {
  ...
  async updateStatsByApiId(
    ...
    const { affected } = await this.statsRepository.update(
      { apiIdClient },
      updateStatsDto,
    );
    return affected;
  ...
  async increaseIncomingMessagesCountToSessionByApiId(
    ...
    const { affected } = await this.statsRepository.increment(
      { apiIdClient },
  ...
  async getClientStatsByApiId(
    apiIdClient: Stats["apiIdClient"],
  ): Promise<Stats> {
    return await this.statsRepository.findOne({ where: { apiIdClient } });
  }

  async getCountStatsByApiId(
    apiIdClient: Stats["apiIdClient"],
  ): Promise<Stats> {
